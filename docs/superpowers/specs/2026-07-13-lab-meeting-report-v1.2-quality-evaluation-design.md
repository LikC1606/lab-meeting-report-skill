...
7. Save output, exit status, duration, model, Codex CLI version, Git commit, prompt hash, run number, skill hash, and a SHA-256 of the isolated provider and Windows sandbox transport configuration. Do not save provider URLs, auth data, headers, or environment values.
